...
class MovieDetailsPage extends StatefulWidget {
  ...
  MovieDetailsPage({@required this.id,@required this.title,@required this.image,@required this.heroTag});
...
class _State extends State<MovieDetailsPage> {
  ...
  _requestData() {
    print(widget.id);
    Api.getMovieDetails(widget.id,(entity){
        if(mounted){
          setState(() {
            _detailData = entity;
          });
    ...
    Api.getMovieStill(widget.id, 6,(entity){
       if(mounted){
         setState(() {
           _stillData = entity;
         });
    ...
    Api.getComments(widget.id,(entity){
      if(mounted){
        setState(() {
          _commentData = entity;
        });
                ...
                Align(
                  ...
                  child: ClipRRect(
                      ...
                      child: GestureDetector(
                        //child: loadNetImage(widget.image,height: 200.0),
                        child: Hero(
                          tag: widget.heroTag ?? '',
                          child: loadNetImage(widget.image,
                              height: 200.0),
                        ),
                        onTap: () {
                          if (_detailData != null) {
                            print(_detailData.mobileUrl);
                            NavigatorUtil.push(context, '${Routes.web}?title=${Uri.encodeComponent(widget.title)}'
                                '&url=${Uri.encodeComponent(_detailData.mobileUrl)}');
                          }
                        },
                      )),
                )
  ...
  Widget _buildBody() {
    if (_detailData != null && _stillData != null) {
        ...
        child: Column(
          children: <Widget>[
            ...
            Padding(
              ...
              child: Text( '评论区',style:TextStyles.black15Bold),
            ),
          ],
        ),
      );
        ...
        itemBuilder: (BuildContext context, int position) {
          return Container(
            ...
            child: GestureDetector(
              ...
              onTap: () {
                NavigatorUtil.push(context, '${Routes.web}?title=${Uri.encodeComponent(data[position].name)}'
                    '&url=${Uri.encodeComponent(data[position].alt)}');
              },
            ),
          );
        });
  ...
  Padding _buildStill() {
    ...
    widget.add(FlatButton(
        padding: const EdgeInsets.all(0),
        onPressed: () {
          NavigatorUtil.push(context, '${Routes.photos}?id=${Uri.encodeComponent(_detailData.id)}'
              '&count=${Uri.encodeComponent('$photoCount')}');
        },
        child: ClipRRect(
          borderRadius: BorderRadius.circular(2),
          child: Container(
